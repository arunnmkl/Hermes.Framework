...
namespace Hermes.Messaging.Security
{
    /// <summary>
    ...
    public class HubAuthorizeAttribute : AuthorizeAttribute
    {
        ...
        public override bool AuthorizeHubConnection(HubDescriptor hubDescriptor, IRequest request)
        {
                ...
                || !string.IsNullOrEmpty(queryStringToken)))
            {
                ...
                var authTicket = new TicketDataFormat(new MachineKeyProtector()).Unprotect(protectedToken);
                HermesPrincipal hermesPrincipal = null;
                //var ticket = Startup.OAuthBearerOptions.AccessTokenFormat.Unprotect(protectedToken);
                if (authTicket != null)
                {
                    hermesPrincipal = new HermesPrincipal(authTicket.Identity);
                }

                if (hermesPrincipal != null)
                {
                    context.User = hermesPrincipal;
                }
                else
                {
                    return false;
        ...
        protected override bool UserAuthorized(IPrincipal user)
        {
            ...
            var principal = new HermesPrincipal(user as ClaimsPrincipal);
